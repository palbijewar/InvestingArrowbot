...
function AdminTable() {
  ...
  const handleAmountUpdate = async (sponsorId) => {
    ...
    setSponsors((prev) =>
      prev.map((s) => (s.sponsor_id === sponsorId ? { ...s, amount_deposited: amount } : s))
    );
  };
  ...
  const handleDematAmountUpdate = async (sponsorId) => {
    ...
    await updateDematAmount(sponsorId, { demat_amount: dematAmount });
    setSponsors((prev) =>
      prev.map((s) => (s.sponsor_id === sponsorId ? { ...s, demat_amount: dematAmount } : s))
    );
  };
  ...
  const handleActionClick = async (action) => {
    ...
    if (action === 'toggle') {
      ...
      if (confirm('Are you sure?')) await handleDelete(selectedSponsor.sponsor_id);
    }
    handleMenuClose();
  };
  ...
  const paginatedSponsors = filteredSponsors.slice((currentPage - 1) * limit, currentPage * limit);
            ...
            <TableBody>
              {paginatedSponsors.map((sponsor) => (
                <TableRow key={sponsor.sponsor_id}>
                  ...
                  <TableCell>
                    <select
                      value={editPackage[sponsor.sponsor_id] ?? sponsor.package ?? ''}
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          setSponsors((prev) =>
                            prev.map((s) => (s.sponsor_id === sponsor.sponsor_id ? { ...s, package: pkg } : s))
                          );
                        } catch (error) {
                  ...
                  <TableCell>
                    <TextField
                      ...
                      size="small"
                      variant="outlined"
                      value={editAmount[sponsor.sponsor_id] ?? sponsor.amount_deposited ?? ''}
                      onChange={(e) => setEditAmount((prev) => ({ ...prev, [sponsor.sponsor_id]: e.target.value }))}
                    />
                  </TableCell>
                  <TableCell>
                    <TextField
                      ...
                      size="small"
                      variant="outlined"
                      value={editDematAmount[sponsor.sponsor_id] ?? sponsor.demat_amount ?? ''}
                      onChange={(e) => setEditDematAmount((prev) => ({ ...prev, [sponsor.sponsor_id]: e.target.value }))}
                    />
                  </TableCell>
                  <TableCell>
                    <Button variant="outlined" size="small" onClick={() => handleViewScreenshot(sponsor.sponsor_id)}>
                      View
                    </Button>
                  </TableCell>
                  <TableCell>
